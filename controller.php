<?php
    ...
    if (isset($_POST['signup'])) {
        ...
        $res = mysqli_query($dbconn, $sql) or die('query failed');
        ...
        if (count($errors) === 0) {
            ...
            $insertInfo = mysqli_query($dbconn, $insertQuery);
    ...
    if (isset($_POST['verify'])) {
        $_SESSION['message'] = "";
        $otp = mysqli_real_escape_string($dbconn, $_POST['otp']);
        $otp_query = "SELECT * FROM usersInfo WHERE code = $otp";
        $otp_result = mysqli_query($dbconn, $otp_query);
    ...
    if (isset($_POST['login'])) {
        ...
        $email_check = mysqli_query($dbconn, $emailQuery);

        if (mysqli_num_rows($email_check) > 0) {
            $passwordQuery = "SELECT * FROM usersInfo WHERE email = '$email' AND password = '$password'";
            $password_check = mysqli_query($dbconn, $passwordQuery);
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        $emailCheckQuery = "SELECT * FROM klientet WHERE email = '$email'";
...
if(isset($_POST['verifyEmail'])){
    $_SESSION['message'] = "";
    $OTPverify = mysqli_real_escape_string($dbconn, $_POST['OTPverify']);
    $verifyQuery = "SELECT * FROM usersInfo WHERE code = $OTPverify";
    $runVerifyQuery = mysqli_query($dbconn, $verifyQuery);
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $newQuery = "UPDATE usersInfo SET code = 0";
            $run = mysqli_query($dbconn,$newQuery);
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            ...
            $updatePass = mysqli_query($dbconn, $updatePassword) or die("Query Failed");
            session_unset($email);
            session_destroy();
            header('location: login.php');
        }
